...
use Wirecard\ElasticEngine\Gateway\Request\WeChatTransactionFactory;
...
class WeChatTransactionFactoryUTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->config->method('getValue')->withConsecutive(
            ...
            [$this->equalTo('sub_merchant_id')],
            [$this->equalTo('sub_merchant_name')]
        )->willReturnOnConsecutiveCalls(0, 'sub_merchant_id', '');
    ...
    {
        $transactionFactory = new WeChatTransactionFactory(
            $this->urlBuilder,
            $this->resolver,
            $this->storeManager,
            new WeChatTransaction(),
            $this->basketFactory,
    ...
    {
        $expected = new WeChatTransaction();
        $expected->setOrderDetail('test@example.com John Doe');

        $expected->setAmount(new Amount(1.01, 'USD'));
        $expected->setNotificationUrl('http://magen.to/frontend/notify?orderId=' . self::ORDER_ID);
        $expected->setRedirect(new Redirect(
            'http://magen.to/frontend/redirect?method=wechat-qrpay',
            'http://magen.to/frontend/cancel?method=wechat-qrpay',
            'http://magen.to/frontend/redirect?method=wechat-qrpay'
        ));

        $subMerchantInfo = new SubMerchantInfo();
        $subMerchantInfo->setMerchantId('sub_merchant_id');
        $expected->setSubMerchantInfo($subMerchantInfo);
