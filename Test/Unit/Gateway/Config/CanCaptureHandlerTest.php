...
class CanCaptureHandlerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $searchCriteria = $this->getMockBuilder(SearchCriteria::class)
            ->disableOriginalConstructor()
            ->getMock();
        $transactionList = $this->getMockBuilder(Collection::class)
            ->disableOriginalConstructor()
            ->getMock();
        $transactionList->method('getAllIds')
            ->willReturn([1, 2]);
        $invalidTransactionList = $this->getMockBuilder(Collection::class)
            ->disableOriginalConstructor()
            ->getMock();
        $invalidTransactionList->method('getAllIds')
            ->willReturn([1, 2]);
        ...
        $transactionExtensionInterface = $this->getMockBuilder(TransactionExtensionInterface::class)
            ->disableOriginalConstructor()
            ->getMock();
        $this->transaction = $this->getMockBuilder(Transaction::class)
            ->disableOriginalConstructor()
            ->getMock();
        $this->notAuthTransaction = $this->getMockBuilder(Transaction::class)
            ->disableOriginalConstructor()
            ->getMock();

        $this->transaction->method('getTxnType')
            ->willReturn('authorization');
        $this->transaction->method('setExtensionAttributes')
            ->with($transactionExtensionInterface)
            ->willReturn($this->transaction);
        $this->notAuthTransaction->method('getTxnType')
            ->willReturn('capture');
        $this->notAuthTransaction->method('setExtensionAttributes')
            ->with($transactionExtensionInterface)
            ->willReturn($this->notAuthTransaction);

        $transactions = [$this->transaction];
        $transactionList->method('getLastItem')
            ->willReturn($this->transaction);
        $transactionList->method('getItems')
            ->willReturn($transactions);
        $transactionList->method('getItemById')
            ->willReturn($this->transaction);
        $transactionRepository = $this->getMockBuilder(Transaction\Repository::class)
            ->disableOriginalConstructor()
            ->getMock();
        $transactionRepository->method('getList')
            ->willReturn($transactionList);

        $filterBuilder = $this->getMockBuilder(FilterBuilder::class)
            ->disableOriginalConstructor()
            ->getMock();
        $filterBuilder->method('setField')
            ->willReturn($filterBuilder);
        ...
        $invalidTransactionList->method('getLastItem')
            ->willReturn($this->notAuthTransaction);
        $invalidTransactionList->method('getItems')
            ->willReturn([$this->notAuthTransaction]);
        $invalidTransactionList->method('getItemById')
            ->willReturn($this->notAuthTransaction);
        $transactionRepository = $this->getMockBuilder(Transaction\Repository::class)
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $orderAdapterInterface = $this->getMockBuilder(OrderAdapterInterface::class)
            ->disableOriginalConstructor()
            ->getMock();
        $orderAdapterInterface->method('getId')
            ->willReturn(1);
    ...
    public function testHandle()
    {
        $orderPayment = $this->getMockBuilder(OrderPaymentInterface::class)
            ->disableOriginalConstructor()
            ->getMock();
        $orderPayment->method('getAmountPaid')
            ->willReturn(1.0);
        $orderPayment->method('getAmountOrdered')
            ->willReturn(2.0);
        $this->payment->method('getPayment')
